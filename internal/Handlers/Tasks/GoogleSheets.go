...
func (h Handler) GetCompletedTask(Sender *telebot.User) ([]domain.Task, error) {
	user, err := h.Users.Get(Sender.ID)
	if err != nil {
		return nil, err
	}
...
func (h Handler) GetUnCompletedTask(Sender *telebot.User) ([]domain.Task, error) {
	user, err := h.Users.Get(Sender.ID)
	if err != nil {
		return nil, err
	}
	...
	user.Tasks = allTasks
	err = h.Users.Update(Sender.ID, user)
	return tasks, err
}
...
func (h Handler) AddTask(msg *telebot.Message) error {
	User, err := h.Users.Get(msg.Sender.ID)
	if err != nil {
		if _, err = h.bot.Send(msg.Sender, "Вы не авторизованы, пройдите авторизацию пожалуйста: /login"); err != nil {
			return err
		}
		return nil
	}
