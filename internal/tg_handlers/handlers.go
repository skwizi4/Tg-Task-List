...
func New(tgbot *telebot.Bot, userHandler Handlers.UserHandler, taskHandler Handlers.TaskHandler, logger logs.GoLogger, errChan *ErrChan.ErrorChannel, Users *domain.ActiveUsers,
	...
	statusTasks *domain.ProcessingChangingStatusTasks, GoogleSheetsApi GoogleSheets.SheetsInterface, ProcessingAddingTask *domain.ProcessingAddingTasks, redis Redis.Redis,
...
func (h Handler) RegistrationHandler(msg *telebot.Message) {
	...
	if h.Users.IfExist(msg.Chat.ID) { //Обработчик случая, когда пользвотель уже имеет аккаунт
	...
	} else if !h.Users.IfExist(msg.Chat.ID) { // Обработчик случае когда пользовтель не авторизирован/ не создан новый аккаунт
		if str, err := h.Redis.GetString(msg.Sender.Username); err != nil {
			fmt.Println(str, err)
			if err = h.UserHandler.CreateUser(msg); err != nil {
				h.errChan.HandleError(err)
			}
...
func (h Handler) ProfileHandler(msg *telebot.Message) {
	...
	CompletedTask, err := h.taskHandler.GetCompletedTask(msg.Chat.ID, msg.Sender)
	if err != nil {
		h.errChan.HandleError(err)
	}
	UncompletedTask, err := h.taskHandler.GetUnCompletedTask(msg.Chat.ID, msg.Sender)
	if err != nil {
		h.errChan.HandleError(err)
	}
	...
	var UserStruct domain.User
	err = json.Unmarshal(value, &UserStruct)
	if err != nil {
		h.errChan.HandleError(err)
	}
	if err := h.UserHandler.GetProfile(UncompletedTaskQuantity, CompletedTaskQuantity, UserStruct.Name, UserStruct.IsSendNotification, UserStruct.FrequencyOfNotifications, msg); err != nil {
		h.errChan.HandleError(err)
	}
...
func (h Handler) RenamingTaskHandler(msg *telebot.Message) {
	...
	tasks, err := h.taskHandler.GetUnCompletedTask(msg.Chat.ID, msg.Sender)
	if err != nil {
		h.errChan.HandleError(err)
	}
