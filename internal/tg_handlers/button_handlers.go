...
func (h Handler) HandleAddTaskButton(c *telebot.Callback) error {
	if !h.Users.IfExist(c.Message.Chat.ID) {
	...
	User := h.Users.GetOrCreate(c.Message.Chat.ID, c.Sender.Username)
...
func (h Handler) HandleChangeNotificationsFrequency(c *telebot.Callback) error {
	value, err := h.Redis.GetBytes(c.Sender.Username)
	if err != nil {
		return err
	}
	...
	UserStruct.FrequencyOfNotifications = Frequency
	UpdatedUser, err := json.Marshal(UserStruct)
	if err != nil {
		return err
	}
	if err = h.Redis.Set(c.Sender.Username, UpdatedUser); err != nil {
		return err
	}
	if _, err = h.tgbot.Send(c.Sender, "Операция выполнена успешно"); err != nil {
		return err
	}
	h.Users.GetOrCreate(c.Message.Chat.ID, c.Sender.Username)
	h.Users.SetNotificationFrequency(c.Message.Chat.ID, Frequency)
	h.Users.UpdateUserNotificationFrequency(c.Message.Chat.ID, Frequency)
	return nil
}
...
func (h Handler) HandleGetTaskCompletedButton(c *telebot.Callback) error {
	Tasks, err := h.taskHandler.GetCompletedTask(c.Message.Chat.ID, c.Sender)
	if err != nil {
		return err
	}
	if Tasks != nil {
...
func (h Handler) HandleGetTaskUnCompletedButton(c *telebot.Callback) error {
	tasks, err := h.taskHandler.GetUnCompletedTask(c.Message.Chat.ID, c.Sender)
	if err != nil {
		return err
	}
	if tasks != nil {
...
func (h Handler) HandleSetNotificationsOffOrOn(c *telebot.Callback) error {
	...
	UserStruct.IsSendNotification = IsSend
	UpdatedUser, err := json.Marshal(UserStruct)
	if err != nil {
		return err
	}
	...
	h.Users.GetOrCreate(c.Message.Chat.ID, c.Sender.Username)
	h.Users.SetNotifications(c.Message.Chat.ID, IsSend)
	return nil
}

func (h Handler) HandleSettingStatusCompletedTaskButton(c *telebot.Callback) error {
	User := h.Users.GetOrCreate(c.Message.Chat.ID, c.Sender.Username)
	err := h.GoogleSheetsApi.RenamingCell(User.SpreadSheetID, c.Data, "Выполнено")
	if err != nil {
		return err
	}
	if _, err = h.tgbot.Send(c.Sender, "Операция выполнена успешно!"); err != nil {
